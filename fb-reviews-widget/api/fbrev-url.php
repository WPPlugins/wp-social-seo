<?php
define('FBREV_USER_AGENT', 'FBREV-WPPlugin/1.0');
define('FBREV_SOCKET_TIMEOUT', 10);

function fbrev_get_query_string($params) {
    $query = '';
...
/*-------------------------------- urlopen --------------------------------*/
function fbrev_urlopen($url, $postdata=false, $file=false) {
    $response = array(
        'data' => '',
        'code' => 0
    );
    ...
    if(function_exists('curl_init')) {
        ...
        _fbrev_curl_urlopen($url, $postdata, $response, $file_name, $file_field);
    } else if(ini_get('allow_url_fopen') && function_exists('stream_get_contents')) {
        _fbrev_fopen_urlopen($url, $postdata, $response, $file_name, $file_field);
    } else {
        _fbrev_fsockopen_urlopen($url, $postdata, $response, $file_name, $file_field);
    }
    return $response;
}

/*-------------------------------- CURL --------------------------------*/
function _fbrev_curl_urlopen($url, $postdata, &$response, $file_name, $file_field) {
    $c = curl_init($url);
    $postdata_str = fbrev_get_query_string($postdata);

    $c_options = array(
        CURLOPT_USERAGENT        => FBREV_USER_AGENT,
        CURLOPT_RETURNTRANSFER    => true,
        CURLOPT_POST            => ($postdata_str ? 1 : 0),
        CURLOPT_HEADER            => true,
        CURLOPT_HTTPHEADER        => array('Expect:'),
        CURLOPT_TIMEOUT         => FBREV_SOCKET_TIMEOUT
    );
    if($postdata) {
    ...
    $response['headers'] = _fbrev_get_response_headers($headers, $response);
    $response['code'] = curl_getinfo($c, CURLINFO_HTTP_CODE);
    curl_close($c);
}

/*-------------------------------- fopen --------------------------------*/
function _fbrev_fopen_urlopen($url, $postdata, &$response, $file_name, $file_field) {
    $params = array();
    if($file_name && $file_field) {
        $boundary = '----------' . md5(time());
        $content = fbrev_get_post_content($boundary, $postdata, $file_name, $file_field);
        $header = fbrev_get_http_headers_for_request($boundary, $content, $file_name, $file_field);

        $params = array('http' => array(
            'method'    => 'POST',
            'header'    => $header,
            'content'    => $content,
            'timeout'    => FBREV_SOCKET_TIMEOUT
        ));
    } else {
        if($postdata) {
            $params = array('http' => array(
                ...
                'content'    => fbrev_get_query_string($postdata),
                'timeout'    => FBREV_SOCKET_TIMEOUT
            ));
        }
    }

    ini_set('user_agent', FBREV_USER_AGENT);
...
/*-------------------------------- fsockpen --------------------------------*/
function _fbrev_fsockopen_urlopen($url, $postdata, &$response, $file_name, $file_field) {
    $buf = '';
    $req = '';
    $length = 0;
    $boundary = '----------' . md5(time());
    $postdata_str = fbrev_get_post_content($boundary, $postdata, $file_name, $file_field);
    ...
    $fp = @fsockopen($url_pieces['host'], $url_pieces['port'], $errno, $errstr, FBREV_SOCKET_TIMEOUT);
    if(!$fp) { return false; }
    ...
    $req .= 'Host: ' . $host . "\r\n";
    $req .=  fbrev_get_http_headers_for_request($boundary, $postdata_str, $file_name, $file_field);
    if($postdata_str) {
        $req .= "\r\n\r\n" . $postdata_str;
    }
    $req .= "\r\n\r\n";
    ...
    $headers = _fbrev_get_response_headers($headers, $response);
...
/*-------------------------------- Helpers --------------------------------*/
function _fbrev_get_response_headers($headers, &$response) {
    $headers = explode("\r\n", $headers);
...
function fbrev_get_post_content($boundary, $postdata, $file_name, $file_field) {
    if(empty($file_name) || empty($file_field)) {
        return fbrev_get_query_string($postdata);
    }
...
function fbrev_get_http_headers_for_request($boundary, $content, $file_name, $file_field) {
    $headers = array();
    $headers[] = 'User-Agent: ' . FBREV_USER_AGENT;
