...
class Grwreview_List_Table extends WP_List_Table_Copy {
    ...
    function __construct() {
        global $status, $page;
        global $wpdb;
        $Lists = $wpdb->get_results('SELECT * FROM  ' . $wpdb->prefix . 'grp_google_review');
        //echo $wpdb->last_query;
        $i = 0;
        foreach ($Lists as $List) {
            $this->example_data[$i]['id'] = $List->id;
            $this->example_data[$i]['google_place_id'] = stripcslashes($List->google_place_id);
            ...
            $this->example_data[$i]['author_url'] = stripcslashes($List->author_url);
            ...
            if ($_GET['paged'] != '') {
                $actions = array(
                    ...
                    'change_status' => sprintf('<a href="?page=%s&action=%s&grwrev=%s&paged=%s" style="color:'.$color.'">'.$status.'</a>', 'wps-google-review-status', 'change_status', $List->id, $_GET['paged']),
                );
            } else {
                $actions = array(
                    'edit' => sprintf('<a href="javascript:void(0);" onclick="quickEdit(\'%s\',\'%s\',this)">Quick edit</a>', $List->tag, $List->id),
                    'change_status' => sprintf('<a href="?page=%s&action=%s&grwrev=%s" style="color:'.$color.'">'.$status.'</a>', 'wps-google-review-status', 'change_status', $List->id),
                );
            }
        ...
        parent::__construct(array(
            'singular' => __('grwreview', 'cpqarealisttable'), //singular name of the listed records
            'plural' => __('grwreviews', 'cpqarealisttable'), //plural name of the listed records
    ...
    function get_columns() {
        $columns = array(
            ...
            'google_place_id' => __('Google place id', 'cpqarealisttable'),
            'rating' => __('Rating', 'cpqarealisttable'),
            'text' => __('Text', 'cpqarealisttable'),
            'time' => __('Time', 'cpqarealisttable'),
            'author_url' => __('Author Url', 'cpqarealisttable'),
    ...
    {   
        ...
        html+='</tr>';
            ...
            function save_quickedit(id,e){
                jQuery.post(ajaxurl,{'id':id,'tag':jQuery('#edit_tag').val(),'action': 'update_google_tag',}, function(data){
